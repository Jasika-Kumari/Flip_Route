...
# Load road network for Ranchi
GRAPH_PATH = "data/graphs/jharkhand/ranchi.graphml"
print("Loading Ranchi road network...")
...
def get_optimized_route(start, end, waypoints, strategy="greedy"):
    ...
    if strategy == "brute":
        best_path_nodes = solve_tsp_exact(G, start_node, waypoint_nodes, end_node)
    else:
        best_path_nodes = solve_tsp_greedy(G, start_node, waypoint_nodes, end_node)
    ...
    for i in range(len(best_path_nodes) - 1):
        path = nx.shortest_path(G, best_path_nodes[i], best_path_nodes[i+1], weight="length")
    ...
    eta_min = total_distance / (35 * 1000 / 60)  # Assuming 35 km/h
...
def solve_tsp_exact(graph, start, waypoints, end):
    # Brute-force all permutations for small sets
    best_path = None
    ...
    for perm in permutations(waypoints):
        ...
        for i in range(len(full_route) - 1):
            segment_dist = nx.shortest_path_length(graph, full_route[i], full_route[i+1], weight="length")
